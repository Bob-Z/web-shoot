...
static int  get_response_page(internal_t * internal)
{
	...
	if(url == NULL) {
		printd(DEBUG_ERROR,"Can't get URL from this engine\n");
		return -1;
	}
...
static char * parse_response_page(internal_t * internal)
{
	...
	substring=strstr(internal->page->data + internal->read_index,"class=\"thumb");
...
static char * engine_get_url(engine_t * engine)
{
	...
	while( url == NULL ) {
		while( internal->page->data == NULL ) {
			printd(DEBUG_PAGE | DEBUG_HTTP,"Reading result page %d\n",internal->page_num);
			res = get_response_page(internal);
			if( res == -1 || internal->page->data == NULL) {
				printd(DEBUG_PAGE | DEBUG_HTTP,"Can not get result page %d, starting back\n",internal->page_num);

				internal->read_index = 0;
				internal->page_num = 0;

				if (first_page == TRUE) {
					printd(DEBUG_PAGE | DEBUG_HTTP,"No URL\n");
				...
				printd(DEBUG_PAGE | DEBUG_HTTP,"Got result page %d\n",internal->page_num-1);
			}
		}

		url =  parse_response_page(internal);
		if( url == NULL ) {
			printd(DEBUG_PAGE | DEBUG_HTTP,"No more URL on page %d\n",internal->page_num);
			internal->page_num++;
			free(internal->page->data);
...
/******************************
 deviantart_engine_init
return 0 if no error
******************************/
int deviantart_engine_init(engine_t * engine,const char * keyword,int size,int filter)
{
	internal_t * internal;
